...
struct DBOptions {
  ...
  // If true, collect RL observation metrics periodically.
  // Default: false
  bool rl_enable_metrics = false;

  // Sampling period in milliseconds for RL metrics collection.
  // Default: 500
  uint64_t rl_metrics_period_ms = 500;

  // If true, log the RL metrics snapshot on each sampling interval.
  // Default: false
  bool rl_log_sampled_metrics = false;

  // If true, enable RL-controlled user write ingress throttling.
  // Requires rl_write_base_rate_bytes_per_sec > 0.
  // Default: false
  bool rl_enable_write_throttling = false;

  // Base user write ingress rate in bytes/sec for RL throttling.
  // Applied rate = base_rate * write_multiplier.
  // Default: 0 (disabled)
  uint64_t rl_write_base_rate_bytes_per_sec = 0;

  // Minimum multiplier allowed for RL ingress control.
  // Default: 0.2
  double rl_write_m_min = 0.2;

  // Maximum allowed change per update for RL multiplier. Set to 0 to disable.
  // Default: 0.1
  double rl_write_delta_max = 0.1;

  // If true, clamp multiplier when write stall is detected.
  // Default: false
  bool rl_write_emergency_override = false;

  // Safe multiplier cap when emergency override is active.
  // Default: 0.5
  double rl_write_emergency_safe_multiplier = 0.5;

  // Enable training-only instrumentation for RL ground-truth labels.
  // Default: false
  bool rl_enable_training_instrumentation = false;
