...
RL_POLLER_SOURCE ?= $(if $(wildcard srocksdb_src/rl_poller.cc),srocksdb_src/rl_poller.cc,rl_poller.cc)
RL_POLLER_OBJECT = $(RL_POLLER_SOURCE:%.cc=$(OBJ_DIR)/%.o)
...
# Extra executables built from custom targets outside TOOLS/BENCHMARKS lists.
CUSTOM_EXECUTABLES = rl_poller
...
# Not practical to exactly match all versions/variants in naming (e.g. debug or not)
	...
	rm -f $(BENCHMARKS) $(TOOLS) $(TESTS) $(PARALLEL_TEST) $(MICROBENCHS) $(CUSTOM_EXECUTABLES)
...
rl_poller: $(RL_POLLER_OBJECT) $(LIBRARY)
	$(AM_LINK)
