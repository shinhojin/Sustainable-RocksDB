...
TS=$(date +%m%d_%H%M%S)
OUTDIR=${OUTDIR:-"$REPO_ROOT/srocksdb_evaluation/agent_pid_l0_${TS}"}
# 12 hours
...
usage() {
  cat <<'USAGE'
Usage: ./run_agent_pid_l0.sh [options] [-- <extra args for run_agent_fifo.sh>]
...
clear_db() {
  if [[ -z "$DB_PATH" || "$DB_PATH" == "/" || "$DB_PATH" == "." ]]; then
    echo "[pid-l0] skip db cleanup: unsafe DB_PATH=$DB_PATH" >&2
    return 1
  fi
  if [[ ! -d "$DB_PATH" ]]; then
    echo "[pid-l0] skip db cleanup: DB_PATH not found ($DB_PATH)" >&2
    return 0
  fi
  echo "[pid-l0] cleaning DB_PATH=$DB_PATH"
  if [[ -n "$SUDO_CMD" ]]; then
...
copy_db_log() {
  ...
  if [[ ! -f "$src" ]]; then
    echo "[pid-l0] WARN: DB LOG not found at $src"
    return 0
  fi
  if cp "$src" "$dst" 2>/dev/null; then
    echo "[pid-l0] copied DB LOG -> $dst"
    return 0
  fi
  if [[ -n "$SUDO_CMD" ]] && $SUDO_CMD test -f "$src"; then
    if $SUDO_CMD cp "$src" "$dst"; then
      $SUDO_CMD chown "$(id -u):$(id -g)" "$dst" 2>/dev/null || true
      echo "[pid-l0] copied DB LOG with sudo -> $dst"
      return 0
    fi
  fi
  echo "[pid-l0] WARN: failed to copy DB LOG from $src"
}
...
echo "[pid-l0] OUTDIR=$OUTDIR"
echo "[pid-l0] DURATION_SEC=$DURATION_SEC"
echo "[pid-l0] DB_PATH=$DB_PATH"
echo "[pid-l0] OPTIONS_FILE=$OPTIONS_FILE"
echo "[pid-l0] RL_POLLER_BIN=$RL_POLLER_BIN"
echo "[pid-l0] M_MIN=$M_MIN M_MAX=$M_MAX"
echo "[pid-l0] PID_L0_TARGET=$PID_L0_TARGET_FILES PID_L0_SCALE=$PID_L0_SCALE_FILES"
echo "[pid-l0] PID_KP=$PID_KP PID_KI=$PID_KI PID_KD=$PID_KD"
echo "[pid-l0] PID_I_MIN=$PID_INTEGRAL_MIN PID_I_MAX=$PID_INTEGRAL_MAX PID_U_MAX=$PID_OUTPUT_MAX"
...
echo "[pid-l0] done"
echo "[pid-l0] agent_log: $OUTDIR/agent_log.csv"
echo "[pid-l0] agent_stdout: $OUTDIR/agent_rl_fifo.log"
echo "[pid-l0] poller_log: $OUTDIR/poller.log"
echo "[pid-l0] metrics: $OUTDIR/rl_metrics.csv"
echo "[pid-l0] resource: $OUTDIR/resource_usage.csv"
echo "[pid-l0] db_LOG: $OUTDIR/db_LOG"
