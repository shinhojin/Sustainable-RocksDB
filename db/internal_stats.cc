    ...
    InternalStats::ppt_name_to_info = {
        ...
        {DB::Properties::kEstimatePendingCompactionBytes,
         {false, nullptr, &InternalStats::HandleEstimatePendingCompactionBytes,
          nullptr, nullptr}},
        {"rocksdb.rl.l0_file_count",
         {false, nullptr, &InternalStats::HandleRLL0FileCount, nullptr,
          nullptr}},
        {"rocksdb.rl.imm_memtable_bytes",
         {false, nullptr, &InternalStats::HandleRLImmMemtableBytes, nullptr,
          nullptr}},
        {"rocksdb.rl.write_lat_p99_us",
         {false, nullptr, &InternalStats::HandleRLWriteLatP99, nullptr,
          nullptr}},
        {"rocksdb.rl.write_in_bytes_per_sec",
         {false, nullptr, &InternalStats::HandleRLWriteInBytesPerSec, nullptr,
          nullptr}},
        {"rocksdb.rl.write_multiplier_prev",
         {true, nullptr, nullptr, nullptr,
          &DBImpl::GetPropertyHandleRLWriteMultiplierPrev}},
        {"rocksdb.rl.write_multiplier_cmd",
         {true, nullptr, nullptr, nullptr,
          &DBImpl::GetPropertyHandleRLWriteMultiplierCmd}},
        {"rocksdb.rl.write_multiplier_applied",
         {true, nullptr, nullptr, nullptr,
          &DBImpl::GetPropertyHandleRLWriteMultiplierApplied}},
        {"rocksdb.rl.write_multiplier_current",
         {true, nullptr, nullptr, nullptr,
          &DBImpl::GetPropertyHandleRLWriteMultiplierCurrent}},
        {"rocksdb.rl.true_pending_flush_bytes",
         {false, nullptr, &InternalStats::HandleRLTruePendingFlushBytes,
          nullptr, nullptr}},
        {"rocksdb.rl.true_pending_compaction_bytes",
         {false, nullptr, &InternalStats::HandleRLTruePendingCompactionBytes,
          nullptr, nullptr}},
        {"rocksdb.rl.training_enabled",
         {false, nullptr, &InternalStats::HandleRLTrainingEnabled, nullptr,
          nullptr}},
        {"rocksdb.rl.metrics_enabled",
         {false, nullptr, &InternalStats::HandleRLMetricsEnabled, nullptr,
          nullptr}},
...
bool InternalStats::HandleRLL0FileCount(uint64_t* value, DBImpl* db,
                                        Version* /*version*/) {
  if (db == nullptr) {
    return false;
  }
  *value = db->rl_metrics_snapshot_.l0_file_count.load();
  return true;
}

bool InternalStats::HandleRLImmMemtableBytes(uint64_t* value, DBImpl* db,
                                             Version* /*version*/) {
  if (db == nullptr) {
    return false;
  }
  *value = db->rl_metrics_snapshot_.imm_memtable_bytes.load();
  return true;
}

bool InternalStats::HandleRLWriteLatP99(uint64_t* value, DBImpl* db,
                                        Version* /*version*/) {
  if (db == nullptr) {
    return false;
  }
  *value = db->rl_metrics_snapshot_.write_lat_p99_us.load();
  return true;
}

bool InternalStats::HandleRLWriteInBytesPerSec(uint64_t* value, DBImpl* db,
                                               Version* /*version*/) {
  if (db == nullptr) {
    return false;
  }
  *value = db->rl_metrics_snapshot_.write_in_bytes_per_sec.load();
  return true;
}

bool InternalStats::HandleRLTruePendingFlushBytes(uint64_t* value, DBImpl* db,
                                                  Version* /*version*/) {
  if (db == nullptr ||
      !db->immutable_db_options_.rl_enable_training_instrumentation) {
    return false;
  }
  *value = db->rl_metrics_snapshot_.true_pending_flush_bytes.load();
  return true;
}

bool InternalStats::HandleRLTruePendingCompactionBytes(uint64_t* value,
                                                       DBImpl* db,
                                                       Version* /*version*/) {
  if (db == nullptr ||
      !db->immutable_db_options_.rl_enable_training_instrumentation) {
  ...
  // Training-only label populated by the RL sampler.
  // May be zero if unavailable.
  *value = db->rl_metrics_snapshot_.true_pending_compaction_bytes.load();
  return true;
}

bool InternalStats::HandleRLTrainingEnabled(uint64_t* value, DBImpl* db,
                                            Version* /*version*/) {
  if (db == nullptr) {
    return false;
  }
  *value = db->immutable_db_options_.rl_enable_training_instrumentation ? 1 : 0;
  return true;
}

bool InternalStats::HandleRLMetricsEnabled(uint64_t* value, DBImpl* db,
                                           Version* /*version*/) {
  if (db == nullptr) {
    return false;
  }
  *value = db->immutable_db_options_.rl_enable_metrics ? 1 : 0;
  return true;
}
