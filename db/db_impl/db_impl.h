...
class DBImpl : public DB {
 public:
  ...
  // Thread-safe RL ingress control APIs (DBImpl-only).
  void SetWriteIngressMultiplier(double multiplier);
  double GetWriteIngressMultiplier() const;
  using DB::Flush;
 ...
 private:
  ...
  void StartRLMetricsSampler();
  void StopRLMetricsSampler();
  void RLMetricsSampler();
  void MaybeRecordWriteIngressBytes(size_t batch_bytes);
  void MaybeThrottleWriteIngress(size_t batch_bytes);
  bool IsRLMetricsEnabled() const;
  bool IsRLIngressEnabled() const;
  bool IsWriteStalledForRLEmergency() const;
  ...
  bool GetPropertyHandleRLWriteMultiplierPrev(std::string* value);
  bool GetPropertyHandleRLWriteMultiplierCmd(std::string* value);
  bool GetPropertyHandleRLWriteMultiplierApplied(std::string* value);
  bool GetPropertyHandleRLWriteMultiplierCurrent(std::string* value);
  ...
  struct RLMetricsSnapshot {
    std::atomic<uint64_t> l0_file_count{0};
    std::atomic<uint64_t> imm_memtable_bytes{0};
    std::atomic<uint64_t> write_lat_p99_us{0};
    std::atomic<uint64_t> write_in_bytes_per_sec{0};
    std::atomic<uint64_t> true_pending_flush_bytes{0};
    std::atomic<uint64_t> true_pending_compaction_bytes{0};
    std::atomic<double> write_multiplier_prev{1.0};
  };

  RLMetricsSnapshot rl_metrics_snapshot_;
  std::atomic<uint64_t> rl_incoming_write_bytes_total_{0};
  std::atomic<double> rl_write_multiplier_{1.0};
  std::atomic<double> rl_write_multiplier_cmd_{1.0};
  std::atomic<bool> rl_metrics_stop_{false};
  std::unique_ptr<port::Thread> rl_metrics_thread_;
  uint64_t rl_metrics_sampling_period_us_ = 0;
  std::unique_ptr<RateLimiter> rl_ingress_limiter_;
