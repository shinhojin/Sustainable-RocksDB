...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
  ...
  if (immutable_db_options_.rl_enable_write_throttling &&
      immutable_db_options_.rl_write_base_rate_bytes_per_sec > 0) {
    rl_ingress_limiter_.reset(NewGenericRateLimiter(
        static_cast<int64_t>(
            immutable_db_options_.rl_write_base_rate_bytes_per_sec),
        100 * 1000, 10, RateLimiter::Mode::kWritesOnly, false, 0));
  }
...
bool DBImpl::IsRLMetricsEnabled() const {
  return immutable_db_options_.rl_enable_metrics &&
         rl_metrics_sampling_period_us_ > 0;
}

bool DBImpl::IsRLIngressEnabled() const {
  return immutable_db_options_.rl_enable_write_throttling &&
         immutable_db_options_.rl_write_base_rate_bytes_per_sec > 0 &&
         rl_ingress_limiter_ != nullptr;
}

bool DBImpl::IsWriteStalledForRLEmergency() const {
  return write_controller_.IsStopped() || error_handler_.IsDBStopped();
}

void DBImpl::MaybeRecordWriteIngressBytes(size_t batch_bytes) {
  if (!IsRLMetricsEnabled() || batch_bytes == 0) {
    return;
  }
  rl_incoming_write_bytes_total_.fetch_add(batch_bytes,
                                           std::memory_order_relaxed);
}

void DBImpl::MaybeThrottleWriteIngress(size_t batch_bytes) {
  if (!IsRLIngressEnabled() || batch_bytes == 0) {
    return;
  }
  RateLimiter* limiter = rl_ingress_limiter_.get();
  if (limiter == nullptr) {
    return;
  }
  int64_t remaining = static_cast<int64_t>(batch_bytes);
  int64_t burst_bytes = limiter->GetSingleBurstBytes();
  if (burst_bytes <= 0) {
    burst_bytes = remaining;
  }
  while (remaining > 0) {
    int64_t request_bytes = std::min(remaining, burst_bytes);
    limiter->Request(request_bytes, Env::IOPriority::IO_USER, stats_);
    remaining -= request_bytes;
  }
}

void DBImpl::StartRLMetricsSampler() {
  ROCKS_LOG_INFO(immutable_db_options_.info_log,
                 "[rl] metrics_enabled=%d training_enabled=%d period_ms=%" PRIu64,
                 immutable_db_options_.rl_enable_metrics ? 1 : 0,
                 immutable_db_options_.rl_enable_training_instrumentation ? 1
                                                                      : 0,
                 immutable_db_options_.rl_metrics_period_ms);
  if (!immutable_db_options_.rl_enable_metrics) {
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   "[rl] sampler disabled (rl_enable_metrics=false)");
    return;
  }
  if (rl_metrics_thread_ != nullptr) {
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   "[rl] sampler disabled (already running)");
    return;
  }
  if (immutable_db_options_.rl_metrics_period_ms == 0) {
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   "[rl] sampler disabled (rl_metrics_period_ms=0)");
    return;
  }
  rl_metrics_sampling_period_us_ = static_cast<uint64_t>(
      immutable_db_options_.rl_metrics_period_ms * 1000);
  if (rl_metrics_sampling_period_us_ == 0) {
    rl_metrics_sampling_period_us_ = 1;
  }
  rl_metrics_stop_.store(false, std::memory_order_release);
  rl_metrics_thread_.reset(
      new port::Thread(&DBImpl::RLMetricsSampler, this));
  ROCKS_LOG_INFO(immutable_db_options_.info_log, "[rl] sampler started");
}

void DBImpl::StopRLMetricsSampler() {
  rl_metrics_stop_.store(true, std::memory_order_release);
  if (rl_metrics_thread_) {
    rl_metrics_thread_->join();
    rl_metrics_thread_.reset();
  }
  rl_metrics_sampling_period_us_ = 0;
}

void DBImpl::RLMetricsSampler() {
  const uint64_t period_us = rl_metrics_sampling_period_us_;
  if (period_us == 0) {
    return;
  }
  uint64_t last_total_bytes =
      rl_incoming_write_bytes_total_.load(std::memory_order_relaxed);
  while (!rl_metrics_stop_.load(std::memory_order_acquire)) {
    const uint64_t sample_start_us =
    ...
    uint64_t pending_compaction_bytes = 0;
    {
      ...
      ColumnFamilyData* cfd =
          default_cf_handle_ ? default_cf_handle_->cfd() : nullptr;
      if (cfd && !cfd->IsDropped()) {
        const auto* vstorage = cfd->current()->storage_info();
        l0_file_count = vstorage->NumLevelFiles(0);
        imm_bytes = cfd->imm()->ApproximateUnflushedMemTablesMemoryUsage();
        if (immutable_db_options_.rl_enable_training_instrumentation) {
          // Training-only label sourced from estimated_compaction_needed_bytes
          // (equivalent to rocksdb.estimate-pending-compaction-bytes).
    ...
    uint64_t write_lat_p99_us = 0;
    uint64_t write_lat_p99_9_us = 0;
    uint64_t write_lat_p99_99_us = 0;
    uint64_t write_lat_p99_999_us = 0;
    if (stats_) {
      // DB_WRITE is the cumulative DB write latency histogram (micros).
      // Windowed p99 is a future extension.
      HistogramData hist{};
      stats_->histogramData(DB_WRITE, &hist);
      write_lat_p99_us = static_cast<uint64_t>(hist.percentile99);
      write_lat_p99_9_us = static_cast<uint64_t>(hist.percentile999);
      write_lat_p99_99_us = static_cast<uint64_t>(hist.percentile9999);
      write_lat_p99_999_us = static_cast<uint64_t>(hist.percentile99999);
    }

    uint64_t total_now =
        rl_incoming_write_bytes_total_.load(std::memory_order_relaxed);
    uint64_t diff_bytes = total_now - last_total_bytes;
    last_total_bytes = total_now;
    uint64_t write_in_bytes_per_sec =
        diff_bytes * 1000000 / period_us;

    rl_metrics_snapshot_.l0_file_count.store(l0_file_count,
                                             std::memory_order_relaxed);
    rl_metrics_snapshot_.imm_memtable_bytes.store(imm_bytes,
                                                  std::memory_order_relaxed);
    rl_metrics_snapshot_.write_lat_p99_us.store(
        write_lat_p99_us, std::memory_order_relaxed);
    rl_metrics_snapshot_.write_in_bytes_per_sec.store(
        write_in_bytes_per_sec, std::memory_order_relaxed);
    rl_metrics_snapshot_.write_multiplier_prev.store(
        rl_write_multiplier_.load(std::memory_order_relaxed),
        std::memory_order_relaxed);

    if (immutable_db_options_.rl_enable_training_instrumentation) {
      rl_metrics_snapshot_.true_pending_flush_bytes.store(
          imm_bytes, std::memory_order_relaxed);
      rl_metrics_snapshot_.true_pending_compaction_bytes.store(
          pending_compaction_bytes, std::memory_order_relaxed);
    }

    if (immutable_db_options_.rl_log_sampled_metrics) {
      const double multiplier =
          rl_write_multiplier_.load(std::memory_order_relaxed);
      if (immutable_db_options_.rl_enable_training_instrumentation) {
        ROCKS_LOG_INFO(
            immutable_db_options_.info_log,
            "RL metrics: l0_files=%" PRIu64
            " imm_bytes=%" PRIu64
            " write_p99_us=%" PRIu64
            " write_p99_9_us=%" PRIu64
            " write_p99_99_us=%" PRIu64
            " write_p99_999_us=%" PRIu64
            " write_in_bytes_per_sec=%" PRIu64
            " m_prev=%.3f"
            " pending_flush_bytes=%" PRIu64
            " pending_compaction_bytes=%" PRIu64,
            l0_file_count, imm_bytes, write_lat_p99_us, write_lat_p99_9_us,
            write_lat_p99_99_us, write_lat_p99_999_us,
            write_in_bytes_per_sec, multiplier, imm_bytes,
            pending_compaction_bytes);
      } else {
        ROCKS_LOG_INFO(
            immutable_db_options_.info_log,
            ...
            " m_prev=%.3f",
            l0_file_count, imm_bytes, write_lat_p99_us, write_lat_p99_9_us,
            write_lat_p99_99_us, write_lat_p99_999_us, write_in_bytes_per_sec,
            multiplier);
      }
    }

    const uint64_t sample_end_us = immutable_db_options_.clock->NowMicros();
    const uint64_t elapsed_us = sample_end_us - sample_start_us;
    if (elapsed_us < period_us) {
      immutable_db_options_.clock->SleepForMicroseconds(period_us - elapsed_us);
    }
  }
}
...
Status DBImpl::SetDBOptions(
  ...
  std::unordered_map<std::string, std::string> filtered_options = options_map;
  bool has_rl_write_multiplier = false;
  double rl_write_multiplier = 1.0;
  auto rl_multiplier_it = filtered_options.find("rl_write_multiplier");
  if (rl_multiplier_it != filtered_options.end()) {
    // External RL control hook: update the write ingress multiplier at runtime.
    has_rl_write_multiplier = true;
    try {
      rl_write_multiplier = ParseDouble(rl_multiplier_it->second);
    } catch (const std::exception& ex) {
      return Status::InvalidArgument(
          "SetDBOptions(), invalid rl_write_multiplier", ex.what());
    }
    filtered_options.erase(rl_multiplier_it);
  }

  if (filtered_options.empty()) {
    if (has_rl_write_multiplier) {
      SetWriteIngressMultiplier(rl_write_multiplier);
    }
    return Status::OK();
  }
  ...
  {
    InstrumentedMutexLock l(&mutex_);
    s = GetMutableDBOptionsFromStrings(mutable_db_options_, filtered_options,
                                       &new_options);
  ...
  if (s.ok() && has_rl_write_multiplier) {
    SetWriteIngressMultiplier(rl_write_multiplier);
  }
...
void DBImpl::SetWriteIngressMultiplier(double multiplier) {
  // multiplier is the requested command (m_cmd). Applied value may differ
  // after clamp/slew/emergency override.
  rl_write_multiplier_cmd_.store(multiplier, std::memory_order_relaxed);
  double m_min = immutable_db_options_.rl_write_m_min;
  const double kMinAllowed = 0.01;
  const double kMaxAllowed = 1.0;
  if (m_min < kMinAllowed) {
    if (immutable_db_options_.info_log != nullptr) {
      ROCKS_LOG_INFO(immutable_db_options_.info_log,
                     "[rl] rl_write_m_min too low (%.3f), clamping to %.2f",
                     m_min, kMinAllowed);
    }
    m_min = kMinAllowed;
  }
  if (m_min > kMaxAllowed) {
    if (immutable_db_options_.info_log != nullptr) {
      ROCKS_LOG_INFO(immutable_db_options_.info_log,
                     "[rl] rl_write_m_min too high (%.3f), clamping to %.2f",
                     m_min, kMaxAllowed);
    }
    m_min = kMaxAllowed;
  }
  ...
  const double prev = rl_write_multiplier_.load(std::memory_order_relaxed);
  const double max_delta = immutable_db_options_.rl_write_delta_max;
  const bool force_min = (multiplier <= m_min);
  if (max_delta > 0.0 && !force_min) {
    if (m > prev + max_delta) {
      m = prev + max_delta;
    } else if (m < prev - max_delta) {
      m = prev - max_delta;
    }
  }

  if (immutable_db_options_.rl_write_emergency_override &&
      IsWriteStalledForRLEmergency()) {
    double safe = immutable_db_options_.rl_write_emergency_safe_multiplier;
    if (safe < m_min) {
      safe = m_min;
    }
    if (safe > 1.0) {
  ...
  rl_write_multiplier_.store(m, std::memory_order_relaxed);
  rl_metrics_snapshot_.write_multiplier_prev.store(m,
                                                   std::memory_order_relaxed);

  if (rl_ingress_limiter_ &&
      immutable_db_options_.rl_write_base_rate_bytes_per_sec > 0) {
    const uint64_t base_rate =
        immutable_db_options_.rl_write_base_rate_bytes_per_sec;
    int64_t new_rate = static_cast<int64_t>(base_rate * m);
    if (new_rate <= 0) {
      new_rate = 1;
    }
    rl_ingress_limiter_->SetBytesPerSecond(new_rate);
  }

  if (immutable_db_options_.info_log != nullptr && prev != m) {
    const uint64_t base_rate =
        immutable_db_options_.rl_write_base_rate_bytes_per_sec;
    const uint64_t applied_rate =
        base_rate > 0 ? static_cast<uint64_t>(base_rate * m) : 0;
    ROCKS_LOG_INFO(
        immutable_db_options_.info_log,
        "[rl] write_multiplier changed: %.3f -> %.3f, bytes_per_sec=%" PRIu64,
        prev, m, applied_rate);
  }
}

double DBImpl::GetWriteIngressMultiplier() const {
  return rl_write_multiplier_.load(std::memory_order_relaxed);
}
...
bool DBImpl::GetPropertyHandleRLWriteMultiplierPrev(std::string* value) {
  assert(value != nullptr);
  // Sampled applied multiplier from the RL snapshot.
  double multiplier =
      rl_metrics_snapshot_.write_multiplier_prev.load(std::memory_order_relaxed);
  *value = std::to_string(multiplier);
  return true;
}

bool DBImpl::GetPropertyHandleRLWriteMultiplierCmd(std::string* value) {
  assert(value != nullptr);
  double multiplier = rl_write_multiplier_cmd_.load(std::memory_order_relaxed);
  *value = std::to_string(multiplier);
  return true;
}

bool DBImpl::GetPropertyHandleRLWriteMultiplierApplied(std::string* value) {
  assert(value != nullptr);
  double multiplier = rl_write_multiplier_.load(std::memory_order_relaxed);
  *value = std::to_string(multiplier);
  return true;
}

bool DBImpl::GetPropertyHandleRLWriteMultiplierCurrent(std::string* value) {
  assert(value != nullptr);
  // Safe: rl_write_multiplier_ is atomic and updated without external locking.
  double multiplier = rl_write_multiplier_.load(std::memory_order_relaxed);
  *value = std::to_string(multiplier);
  return true;
}
