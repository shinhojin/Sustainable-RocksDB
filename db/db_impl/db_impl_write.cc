...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  if (IsRLMetricsEnabled() || IsRLIngressEnabled()) {
    const size_t batch_bytes = WriteBatchInternal::ByteSize(my_batch);
    MaybeRecordWriteIngressBytes(batch_bytes);
    MaybeThrottleWriteIngress(batch_bytes);
  }
